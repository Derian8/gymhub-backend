...
def test_trainer_overview_returns_members_at_risk(
    ...
    assert 'payments_overdue' not in response.data
    assert 'active_subscriptions_count' not in response.data
    assert 'estimated_mrr' not in response.data
    assert 'expected_revenue_this_month' not in response.data
    assert 'late_rate_pct' not in response.data
