...
3. Ejecuta `./gym-start` para levantar `frontend`, `backend`, `db`, `redis`, `celery` y `celerybeat`.
   Para un arranque de producción local usa `./gym-start --prod`.
4. Usa `./gym-log` para seguir logs en tiempo real.
5. Ejecuta `./gym-smoke` para validar el MVP con usuarios demo y endpoints reales.
6. Usa `./gym-stop` para detener los contenedores sin borrar volúmenes.
7. Usa `./gym-frontend-test` para correr Vitest del frontend dentro del contenedor dedicado `frontend-test`, sin depender de `frontend/node_modules` del host.
