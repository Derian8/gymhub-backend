...
describe('BillingPage', () => {
  ...
  it('renders billing summary, payment rows and membership portfolio without plan catalog', () => {
    ...
    fireEvent.click(getByText('Historial de cobros'))
    ...
    fireEvent.click(getByText('Cartera'))
  ...
  it('shows member-specific header when member filter is present', () => {
    ...
    expect(getByTestId('membership-mode-catalog')).toHaveTextContent('Membresía del catálogo')
    expect(getByTestId('membership-mode-custom')).toHaveTextContent('Membresía personalizada')
  ...
  it('creates a custom commercial membership without a training plan', () => {
    const { getByRole, getByTestId } = renderWithProviders(<BillingPage />, { route: '/billing?member=15' })

    fireEvent.click(getByTestId('membership-mode-custom'))
    fireEvent.change(getByTestId('custom-membership-name'), { target: { value: 'Convenio corporativo' } })
    fireEvent.change(getByTestId('custom-membership-price'), { target: { value: '25000' } })
    fireEvent.click(getByRole('button', { name: 'Asignar membresía y crear primer cobro' }))
